...
class MessageContentType {
  ...
  // Reaction message type (not displayed, used for broadcasting reactions)
  static const String reaction = 'reaction'; // broadcast only
  ...
  static const String privateGroupInvitation =
      'privateGroup:invitation'; // db + visible
  static const String privateGroupAccept = 'privateGroup:accept'; // .
  static const String privateGroupSubscribe =
      'privateGroup:subscribe'; // db + visible
  static const String privateGroupQuit = 'privateGroup:quit'; // .
  static const String privateGroupOptionRequest =
      'privateGroup:optionRequest'; // .
  static const String privateGroupOptionResponse =
      'privateGroup:optionResponse'; // .
  static const String privateGroupMemberRequest =
      'privateGroup:memberRequest'; // .
  static const String privateGroupMemberResponse =
...
class MessageSchema {
  ...
  bool get isTargetSelf {
    return isTargetContact &&
        (isOutbound
            ? (sender == targetId)
            : ((sender == clientCommon.address) &&
                (clientCommon.address?.isNotEmpty == true)));
  }

  // burning
  bool get canBurning {
    bool isText = contentType == MessageContentType.text ||
        contentType == MessageContentType.textExtension;
  ...
  // ++ session
  bool get canDisplay {
    bool isEvent = contentType == MessageContentType.contactOptions ||
        contentType == MessageContentType.topicSubscribe ||
        contentType ==
            MessageContentType
                .privateGroupSubscribe; // || contentType == MessageContentType.topicUnsubscribe || contentType == MessageContentType.topicKickOut
    return canNotification || isEvent;
  }

  bool get isTopicAction {
    return contentType == MessageContentType.topicSubscribe ||
  ...
  bool get isGroupAction {
    bool isAction = contentType == MessageContentType.privateGroupInvitation ||
        contentType == MessageContentType.privateGroupAccept ||
        contentType == MessageContentType.privateGroupSubscribe ||
        contentType == MessageContentType.privateGroupQuit;
    bool isSync = contentType == MessageContentType.privateGroupOptionRequest ||
        contentType == MessageContentType.privateGroupOptionResponse ||
        contentType == MessageContentType.privateGroupMemberRequest ||
        contentType == MessageContentType.privateGroupMemberResponse;
  ...
  static MessageSchema? fromReceive(OnMessage? raw) {
    ...
    String targetId =
        topic.isNotEmpty ? topic : (groupId.isNotEmpty ? groupId : sender);
    int targetType = topic.isNotEmpty
        ? SessionType.TOPIC
        : (groupId.isNotEmpty
    ...
    MessageSchema schema = MessageSchema(
      ...
      // at
      sendAt: int.tryParse(data['send_timestamp']?.toString() ?? "") ??
    ...
    schema.status = schema.canReceipt
        ? (schema.isTargetContact ? schema.status : MessageStatus.Receipt)
        : MessageStatus.Read;
    // content
  ...
  static MessageSchema fromSend(
    ...
    if (profileVersion != null && profileVersion.isNotEmpty) {
      schema.options =
          MessageOptions.setProfileVersion(schema.options, profileVersion);
    }
    String? deviceToken = extra?["deviceToken"];
    if (deviceToken != null && deviceToken.isNotEmpty) {
      schema.options =
          MessageOptions.setDeviceToken(schema.options, deviceToken);
    }
    String? deviceProfile = extra?["deviceProfile"];
    if (deviceProfile != null && deviceProfile.isNotEmpty) {
      schema.options =
          MessageOptions.setDeviceProfile(schema.options, deviceProfile);
    }
    String? privateGroupVersion = extra?["privateGroupVersion"];
    if (privateGroupVersion != null && privateGroupVersion.isNotEmpty) {
      schema.options = MessageOptions.setPrivateGroupVersion(
          schema.options, privateGroupVersion);
    }
    int? deleteAfterSeconds = extra?["deleteAfterSeconds"];
    if (deleteAfterSeconds != null && deleteAfterSeconds > 0) {
      schema.options = MessageOptions.setOptionsBurningDeleteSec(
          schema.options, deleteAfterSeconds);
    }
    int? burningUpdateAt = extra?["burningUpdateAt"];
    if (burningUpdateAt != null && burningUpdateAt > 0) {
      schema.options = MessageOptions.setOptionsBurningUpdateAt(
          schema.options, burningUpdateAt);
    }
    String? queueIds = extra?["queueIds"];
    if (queueIds != null && queueIds.isNotEmpty) {
      schema.options =
          MessageOptions.setMessageQueueIds(schema.options, queueIds);
    }
    ...
    if (fileMimeType != null && fileMimeType.isNotEmpty) {
      schema.options =
          MessageOptions.setFileMimeType(schema.options, fileMimeType);
    }
    ...
    if (fileType != null && fileType >= 0) {
      schema.options = MessageOptions.setFileType(schema.options, fileType);
        ...
        (fileMimeType?.isNotEmpty == true) ||
        (fileExt?.isNotEmpty == true)) {
      if ((fileMimeType?.contains("image") == true)) {
        schema.options = MessageOptions.setFileType(
            schema.options, MessageOptions.fileTypeImage);
      } else if ((fileMimeType?.contains("audio") == true)) {
        schema.options = MessageOptions.setFileType(
            schema.options, MessageOptions.fileTypeAudio);
      } else if ((fileMimeType?.contains("video") == true)) {
        schema.options = MessageOptions.setFileType(
            schema.options, MessageOptions.fileTypeVideo);
      } else {
        // file_picker is here, because no mime_type
        schema.options = MessageOptions.setFileType(
            schema.options, MessageOptions.fileTypeNormal);
      }
    }
    ...
    if (mediaWidth != null &&
        ...
        mediaHeight != null &&
        mediaHeight != 0) {
      schema.options = MessageOptions.setMediaSizeWH(
          schema.options, mediaWidth, mediaHeight);
    }
    double? duration = double.tryParse(extra?["duration"]?.toString() ?? "");
    if (duration != null && duration >= 0) {
      schema.options =
          MessageOptions.setMediaDuration(schema.options, duration);
      // SUPPORT:START
    ...
    if (thumbnailPath != null && thumbnailPath.isNotEmpty) {
      schema.options =
          MessageOptions.setMediaThumbnailPath(schema.options, thumbnailPath);
    }
    // piece
  ...
  static MessageSchema fromMap(Map<String, dynamic> e) {
    MessageSchema schema = MessageSchema(
      ...
      // status
      isOutbound:
          (e['is_outbound'] != null && e['is_outbound'] == 1) ? true : false,
      status: e['status'] ?? 0,
      // at
      ...
      // data
      contentType: e['type'] ?? "",
      options: (e['options']?.toString().isNotEmpty == true)
          ? Util.jsonFormatMap(e['options'])
          : null,
      data: e['data'],
    );
    // content
    switch (schema.contentType) {
      ...
      case MessageContentType.deviceRequest:
      case MessageContentType.deviceInfo:
        if ((e['content']?.toString().isNotEmpty == true) &&
            (e['content'] is String)) {
      ...
      case MessageContentType.piece:
        ...
        schema.content =
            (completePath?.isNotEmpty == true) ? File(completePath!) : null;
        break;
      case MessageContentType.privateGroupInvitation:
      case MessageContentType.privateGroupAccept:
      case MessageContentType.privateGroupQuit:
  ...
  static Future<Map<String, dynamic>> piecesSplits(MessageSchema msg) async {
    ...
    if (bytesLength < (Settings.piecesPreMinLen * Settings.piecesMinTotal)) {
      return {};
        ...
        (Settings.piecesPreMinLen * Settings.piecesMaxTotal)) {
    ...
    // parity(1~63)
    int parity = (total *
            (Settings.piecesMaxParity /
                (Settings.piecesMaxTotal + Settings.piecesMaxParity)))
        .toInt();
    if (total %
            (Settings.piecesMaxParity /
                (Settings.piecesMaxTotal + Settings.piecesMaxParity)) >
  ...
  static Future<String?> combinePiecesData(List<MessageSchema> pieces,
      int total, int parity, int bytesLength) async {
    ...
    for (int i = 0; i < pieces.length; i++) {
      ...
      if (file == null || !file.existsSync()) {
        logger.e(
            "Message - combinePiecesData - combine sub_file no exists - item:$item - file:${file?.path}");
        continue;
      }
      Uint8List itemBytes = file.readAsBytesSync();
      int? pieceIndex = item.options?[MessageOptions.KEY_PIECE_INDEX];
      if (itemBytes.isNotEmpty &&
          (pieceIndex != null) &&
          (pieceIndex >= 0) &&
          (pieceIndex < recoverList.length)) {
    ...
    if (recoverCount < total) {
      logger.e(
          "Message - combinePiecesData - combine fail - recover_lost:${pieces.length - recoverCount}");
      return null;
    }
    return await Common.combinePieces(recoverList, total, parity, bytesLength);
  }

  static MessageSchema? combinePiecesMsg(
      List<MessageSchema> sortPieces, String base64String) {
    List<MessageSchema> finds = sortPieces.where((element) {
      bool pid = element.pid != null;
      bool contentType =
          (element.options?[MessageOptions.KEY_PIECE_PARENT_TYPE]?.toString() ??
                  "")
              .isNotEmpty;
...
class MessageOptions {
  ...
  static const KEY_IPFS_THUMBNAIL_ENCRYPT_ALGORITHM =
      "ipfsThumbnailEncryptAlgorithm";
  static const KEY_IPFS_THUMBNAIL_ENCRYPT_KEY_BYTES =
      "ipfsThumbnailEncryptKeyBytes";
  static const KEY_IPFS_THUMBNAIL_ENCRYPT_NONCE_SIZE =
      "ipfsThumbnailEncryptNonceSize";
  ...
  static Map<String, dynamic> setSendSuccessAt(
      Map<String, dynamic>? options, int sendAt) {
  ...
  static int? getSendSuccessAt(Map<String, dynamic>? options) {
    ...
    return int.tryParse(
        options[MessageOptions.KEY_SEND_SUCCESS_AT]?.toString() ?? "");
  }

  static Map<String, dynamic> setResendMuteAt(
      Map<String, dynamic>? options, int resendAt) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static int? getResendMuteAt(Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return null;
    return int.tryParse(
        options[MessageOptions.KEY_RESEND_MUTE_AT]?.toString() ?? "");
  }

  static Map<String, dynamic> setSuccessTimes(
      Map<String, dynamic>? options, int resendAt) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static int? getSuccessTimes(Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return null;
    return int.tryParse(
        options[MessageOptions.KEY_SUCCESS_TIMES]?.toString() ?? "");
  }

  static Map<String, dynamic> setProfileVersion(
      Map<String, dynamic>? options, String profileVersion) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setDeviceToken(
      Map<String, dynamic>? options, String deviceToken) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setDeviceProfile(
      Map<String, dynamic>? options, String deviceProfile) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setFirstMessageQueueIds(
      Map<String, dynamic>? options, String queueIds) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setMessageQueueIds(
      Map<String, dynamic>? options, String queueIds) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setPrivateGroupVersion(
      Map<String, dynamic>? options, String peivateGroupVersion) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setOptionsBurningDeleteSec(
      Map<String, dynamic>? options, int deleteTimeSec) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setOptionsBurningUpdateAt(
      Map<String, dynamic>? options, int? updateAt) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static int? getOptionsBurningUpdateAt(Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return null;
    var update =
        options[MessageOptions.KEY_UPDATE_BURNING_AFTER_AT]?.toString();
    return int.tryParse(update ?? "");
  }

  static Map<String, dynamic> setPushNotifyId(
      Map<String, dynamic>? options, String uuid) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setFileType(
      Map<String, dynamic>? options, int type) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setFileName(
      Map<String, dynamic>? options, String? ext) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setFileSize(
      Map<String, dynamic>? options, int? size) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setFileExt(
      Map<String, dynamic>? options, String? ext) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setFileMimeType(
      Map<String, dynamic>? options, String? mimeType) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setMediaSizeWH(
      Map<String, dynamic>? options, int? width, int? height) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setMediaDuration(
      Map<String, dynamic>? options, double? durationS) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setMediaThumbnailPath(
      Map<String, dynamic>? options, String? thumbnailPath) {
    if (options == null) options = Map<String, dynamic>();
    options[MessageOptions.KEY_MEDIA_THUMBNAIL] =
        Path.convert2Local(thumbnailPath);
    return options;
  }

  static String? getMediaThumbnailPath(Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return null;
    return Path.convert2Complete(
        options[MessageOptions.KEY_MEDIA_THUMBNAIL]?.toString());
  }

  static Map<String, dynamic> setIpfsState(
      Map<String, dynamic>? options, int state) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static Map<String, dynamic> setIpfsThumbnailState(
      Map<String, dynamic>? options, int state) {
    if (options == null) options = Map<String, dynamic>();
  ...
  static int? getIpfsEncryptNonceSize(Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return null;
    var nonceLen =
        options[MessageOptions.KEY_IPFS_ENCRYPT_NONCE_SIZE]?.toString();
  ...
  static Map<String, dynamic> setIpfsResultThumbnail(
    ...
    options[MessageOptions.KEY_IPFS_THUMBNAIL_ENCRYPT_ALGORITHM] =
        encryptAlgorithm;
    options[MessageOptions.KEY_IPFS_THUMBNAIL_ENCRYPT_KEY_BYTES] = encryptKey;
    options[MessageOptions.KEY_IPFS_THUMBNAIL_ENCRYPT_NONCE_SIZE] =
        encryptNonce;
    return options;
  }
  ...
  static bool getIpfsThumbnailEncrypt(Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return false;
    var encrypt =
        options[MessageOptions.KEY_IPFS_THUMBNAIL_ENCRYPT]?.toString();
    if (encrypt == null || encrypt.isEmpty) return false;
    return (int.tryParse(encrypt) ?? 0) > 0 ? true : false;
  }

  static String? getIpfsThumbnailEncryptAlgorithm(
      Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return null;
    return options[MessageOptions.KEY_IPFS_THUMBNAIL_ENCRYPT_ALGORITHM]
        ?.toString();
  }

  static Uint8List? getIpfsThumbnailEncryptKeyBytes(
      Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return null;
  ...
  static int? getIpfsThumbnailEncryptNonceSize(Map<String, dynamic>? options) {
    if (options == null || options.keys.length == 0) return null;
    var nonceLen = options[MessageOptions.KEY_IPFS_THUMBNAIL_ENCRYPT_NONCE_SIZE]
        ?.toString();
    if (nonceLen == null || nonceLen.isEmpty) return null;
    return int.tryParse(nonceLen);
  }
...
class MessageData {
  static Map _base(String contentType,
      {String? id, int? timestamp, int? queueId}) {
    Map map = {
  ...
  static String getContactProfileRequest(
      String requestType, String? profileVersion) {
    Map data = _base(
        MessageSchema.supportContentType(MessageContentType.contactProfile));
    data.addAll({
  ...
  static Future<String> getContactProfileResponseFull(String? profileVersion,
      File? avatar, String? firstName, String? lastName) async {
    Map data = _base(
        MessageSchema.supportContentType(MessageContentType.contactProfile));
    data.addAll({
    ...
    if (avatar != null && await avatar.exists()) {
      ...
      if (base64.isNotEmpty == true) {
        content['avatar'] = {
          'type': 'base64',
          'data': base64,
          'ext': Path.getFileExt(avatar, FileHelper.DEFAULT_IMAGE_EXT)
        };
      }
    }
  ...
  static String getContactOptionsBurn(
      String msgId, int? burnAfterSeconds, int? updateBurnAfterAt) {
    Map data = _base(
        MessageSchema.supportContentType(MessageContentType.contactOptions),
        id: msgId);
    data.addAll({
      'optionType': '0',
      'content': {
  ...
  static String getText(MessageSchema message) {
    Map data = _base(message.contentType,
        id: message.msgId, timestamp: message.sendAt, queueId: message.queueId);
    data.addAll({
  ...
  static Future<String?> getImage(MessageSchema message) async {
    ...
    Map data = _base(MessageSchema.supportContentType(message.contentType),
        id: message.msgId, timestamp: message.sendAt, queueId: message.queueId);
    data.addAll({
      'content': content,
      'options': _simpleOptions(message.options),
  ...
  static String getTopicSubscribe(String? msgId, String topicId) {
    Map data = _base(
        MessageSchema.supportContentType(MessageContentType.topicSubscribe),
        id: msgId);
    data.addAll({
      'topic': topicId,
    });
  ...
  static String getTopicUnSubscribe(String topicId) {
    Map data = _base(
        MessageSchema.supportContentType(MessageContentType.topicUnsubscribe));
    data.addAll({
      'topic': topicId,
    });
    return jsonEncode(data);
  ...
  static String getTopicInvitee(MessageSchema message) {
    Map data = _base(MessageSchema.supportContentType(message.contentType),
        id: message.msgId, timestamp: message.sendAt);
    data.addAll({
      'content': message.content,
    });
    return jsonEncode(data);
  ...
  static String getTopicKickOut(String topic, String targetId) {
    Map data = _base(
        MessageSchema.supportContentType(MessageContentType.topicKickOut));
    data.addAll({
      'topic': topic,
      'content': targetId,
    });
  ...
  static String getPrivateGroupInvitation(MessageSchema message) {
    Map data = _base(MessageContentType.privateGroupInvitation,
        id: message.msgId, timestamp: message.sendAt);
    data.addAll({
      'content': message.content,
    });
  ...
  static String getPrivateGroupOptionRequest(
      String groupId, String? privateGroupVersion) {
  ...
  static String getPrivateGroupMemberRequest(
      String groupId, String? privateGroupVersion) {
  ...
  static String getPrivateGroupMemberResponse(
      PrivateGroupSchema privateGroup, List<Map<String, dynamic>> membersData) {
