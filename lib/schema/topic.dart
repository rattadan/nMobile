...
class TopicSchema {
  ...
  TopicSchema({
    ...
    if (type == -1)
      type = (Validate.isPrivateTopicOk(topicId)
          ? TopicType.private
          : TopicType.public);
  }

  static TopicSchema? create(String? topicId,
      {int? type, int expireHeight = 0}) {
    if (topicId == null || topicId.isEmpty) return null;
    return TopicSchema(
      topicId: topicId,
      type: type ??
          (Validate.isPrivateTopicOk(topicId)
              ? TopicType.private
              : TopicType.public),
      ...
      subscribeAt:
          expireHeight > 0 ? DateTime.now().millisecondsSinceEpoch : null,
      ...
      if (index > 0) {
        ...
        if (ownerPubKey?.isNotEmpty == true) {
          if ((ownerPubKey?.length ?? 0) > 8) {
            topicNameShort =
                topicName + '.' + (ownerPubKey?.substring(0, 8) ?? "");
          } else {
  ...
  bool isOwner(String? contactAddress) {
    if (!isPrivate || contactAddress == null || contactAddress.isEmpty)
      return false;
  ...
  Future<bool> shouldResubscribe(int? globalHeight) async {
    if ((expireBlockHeight == null) || ((expireBlockHeight ?? 0) <= 0))
      return true;
    globalHeight = globalHeight ?? (await RPC.getBlockHeight());
    if (globalHeight != null && globalHeight > 0) {
      return ((expireBlockHeight ?? 0) - globalHeight) <
  ...
  int? getProgressSubscribeNonce() {
    int? nonce =
        int.tryParse(data['progress_subscribe_nonce']?.toString() ?? "");
  ...
  double getProgressSubscribeFee() {
    double? fee =
        double.tryParse(data['progress_subscribe_fee']?.toString() ?? "");
    if (fee == null || fee < 0) return 0;
    return fee;
  }

  int lastCheckSubscribeAt() {
    return int.tryParse(data['last_check_subscribe_at']?.toString() ?? "0") ??
        0;
  }

  int lastCheckPermissionsAt() {
    return int.tryParse(data['last_check_permissions_at']?.toString() ?? "0") ??
        0;
  }

  int lastRefreshSubscribersAt() {
    return int.tryParse(
            data['last_refresh_subscribers_at']?.toString() ?? "0") ??
        0;
  }

  // Avatar broadcast tracking
  int get lastAvatarBroadcastAt {
    return int.tryParse(data['last_avatar_broadcast_at']?.toString() ?? "0") ??
        0;
  }

  set lastAvatarBroadcastAt(int value) {
    data['last_avatar_broadcast_at'] = value;
  }

  int get lastAvatarReceivedAt {
    return int.tryParse(data['last_avatar_received_at']?.toString() ?? "0") ??
        0;
  }

  set lastAvatarReceivedAt(int value) {
    data['last_avatar_received_at'] = value;
  }

  String? get cachedAvatarBase64 {
    return data['cached_avatar_base64']?.toString();
  }

  set cachedAvatarBase64(String? value) {
    data['cached_avatar_base64'] = value;
  }
  ...
  static TopicSchema fromMap(Map<String, dynamic> e) {
    var topicSchema = TopicSchema(
      ...
      avatar: Path.convert2Complete(e['avatar']) != null
          ? File(Path.convert2Complete(e['avatar'])!)
          : null,
      count: e['count'] ?? 0,
