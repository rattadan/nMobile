...
class ContactAddScreenState extends BaseStateFulWidgetState<ContactAddScreen>
    with Tag {
  ...
  _selectAvatarPicture() async {
    String savePath = await Path.getRandomFile(
        ...
        subPath: null, fileExt: FileHelper.DEFAULT_IMAGE_EXT);
    application.inSystemSelecting = true;
    File? picked = await MediaPicker.pickImage(
      cropRectangle: true, // rectangle crop
  ...
  formatQrDate(String? clientAddress) async {
    ...
    logger.i(
        "$TAG - QR_DATA_DECODE - clientAddress:$clientAddress - walletAddress:$walletAddress");
  ...
  _saveContact(BuildContext context) async {
    if ((_formKey.currentState as FormState).validate()) {
      ...
      // input
      String address =
          _clientAddressController.text.replaceAll("\n", "").trim();
      String? remarkAvatar = _headImage?.path.isNotEmpty == true
          ? Path.convert2Local(_headImage?.path)
          : null;
      String? remarkName =
      ...
      if ((exist != null) && (exist.type == ContactType.friend)) {
        ...
        bool success = await contactCommon
            .setType(exist.address, ContactType.friend, notify: true);
        if (success) exist.type = ContactType.friend;
        success = await contactCommon
            .setOtherRemarkName(exist.address, remarkName, notify: true);
        if (success) exist.remarkName = remarkName ?? "";
        var data = await contactCommon
            .setOtherRemarkAvatar(exist.address, remarkAvatar, notify: true);
        ...
        ContactSchema? schema =
            ContactSchema.create(clientAddress, ContactType.friend);
                      ...
                      children: <Widget>[
                        ...
                        FormText(
                          ...
                          onFieldSubmitted: (_) => FocusScope.of(context)
                              .requestFocus(_clientAddressFocusNode),
                        ),
                        SizedBox(height: 14),
                        Label(
                        ...
                        FormText(
                          ...
                          hintText: Settings.locale(
                              (s) => s.input_d_chat_address,
                              ctx: context),
                          validator: Validator.of(context).pubKeyNKN(),
                          textInputAction: TextInputAction.next,
                          onFieldSubmitted: (_) => FocusScope.of(context)
                              .requestFocus(_walletAddressFocusNode),
                          // multi: true,
                          maxLines: 10,
                        ),
                        Row(
                          children: <Widget>[
                            Label(
                              Settings.locale((s) => s.wallet_address,
                                  ctx: context),
                              type: LabelType.h3,
                              textAlign: TextAlign.start,
                            ),
                        ...
                        FormText(
                          ...
                          hintText: Settings.locale((s) => s.input_notes,
                              ctx: context),
                ...
                child: Column(
                  children: <Widget>[
                    Padding(
                      ...
                      child: Button(
                        text: Settings.locale((s) => s.save_contact,
                            ctx: context),
