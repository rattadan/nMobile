...
import 'package:nchat_mobile/components/text/fixed_text_field.dart';
...
import 'package:nchat_mobile/screens/contact/home_empty.dart';
...
class ContactHomeScreen extends BaseStateFulWidget {
  ...
  static Future go(BuildContext? context,
      {String? title, bool selectContact = false, bool selectGroup = false}) {
...
class _ContactHomeScreenState
    extends BaseStateFulWidgetState<ContactHomeScreen> {
  String _navTitle = "";
  ...
  void onRefreshArguments() {
    ...
    this._selectContact =
        widget.arguments?[ContactHomeScreen.argSelectContact] ?? false;
    this._selectGroup =
        widget.arguments?[ContactHomeScreen.argSelectGroup] ?? false;
  ...
  void initState() {
    super.initState();
    // contact listen
    _addContactSubscription =
        contactCommon.addStream.listen((ContactSchema schema) {
      if (schema.type == ContactType.friend) {
        if (_allFriends
                .indexWhere((element) => element.address == schema.address) <
            0) {
    ...
    //   _searchAction(_searchController.text);
    // });
    _updateContactSubscription =
        contactCommon.updateStream.listen((ContactSchema event) {
      // friend
      int friendIndex = -1;
    ...
    _addTopicSubscription = topicCommon.addStream.listen((TopicSchema schema) {
      if (_allTopics
              .indexWhere((element) => element.topicId == schema.topicId) <
          0) {
    ...
    // });
    _updateContactSubscription =
        topicCommon.updateStream.listen((TopicSchema event) {
      _allTopics = _allTopics
          .map((e) => e.topicId == event.topicId ? event : e)
          .toList();
      if (!event.joined) {
        _allTopics = _allTopics
            .where((element) => element.topicId != event.topicId)
            .toList();
      }
      _searchAction(_searchController.text);
    });
    ...
    // group listen
    _addGroupSubscription =
        privateGroupCommon.addGroupStream.listen((PrivateGroupSchema schema) {
      if (_allGroups
              .indexWhere((element) => element.groupId == schema.groupId) <
          0) {
    ...
    _updateGroupSubscription =
        privateGroupCommon.updateGroupStream.listen((PrivateGroupSchema event) {
      _allGroups = _allGroups
          .map((e) => e.groupId == event.groupId ? event : e)
          .toList();
      if (!event.joined) {
        _allGroups = _allGroups
  ...
  _initData() async {
    ...
    for (int offset = 0; true; offset += limit) {
      List<ContactSchema> result = await contactCommon.queryList(
          type: ContactType.friend, offset: offset, limit: limit);
    ...
    for (int offset = 0; true; offset += limit) {
      List<TopicSchema> result =
          await topicCommon.queryListJoined(offset: offset, limit: limit);
      topics.addAll(result);
      if (result.length < limit) break;
    }
    topics = (!this._isSelect || this._selectGroup)
        ? topics
        : []; // can not move this line to setState
    // group
    List<PrivateGroupSchema> groups = [];
    for (int offset = 0; true; offset += limit) {
      List<PrivateGroupSchema> result = await privateGroupCommon
          .queryGroupListJoined(offset: offset, limit: limit);
      groups.addAll(result);
      if (result.length < limit) break;
    }
    groups = (!this._isSelect || this._selectGroup)
        ? groups
        : []; // can not move this line to setState
    // strangers
      ...
      setState(() {
        ...
        _searchFriends = _allFriends
            .where((ContactSchema e) =>
                e.displayName.toLowerCase().contains(val.toLowerCase()))
            .toList();
        _searchTopics = _allTopics
            .where((TopicSchema e) => e.topicId.contains(val))
            .toList();
        _searchGroups = _allGroups
            .where((PrivateGroupSchema e) => e.name.contains(val))
            .toList();
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    int totalDataCount = totalFriendDataCount +
        totalTopicDataCount +
        totalGroupDataCount; // + totalStrangerDataCount;
    if (totalDataCount <= 0 && _pageLoaded) {
      return ContactHomeEmptyLayout();
    }

    int searchFriendDataCount = _searchFriends.length;
    int searchFriendViewCount =
        (searchFriendDataCount > 0 ? 1 : 0) + searchFriendDataCount;
    int searchTopicDataCount = _searchTopics.length;
    int searchTopicViewCount =
        (searchTopicDataCount > 0 ? 1 : 0) + searchTopicDataCount;
    int searchGroupDataCount = _searchGroups.length;
    int searchGroupViewCount =
        (searchGroupDataCount > 0 ? 1 : 0) + searchGroupDataCount;
    ...
    int listItemViewCount = searchFriendViewCount +
        searchTopicViewCount +
        searchGroupViewCount; //  + searchStrangerViewCount;
    ...
    return Layout(
      ...
      header: Header(
        title: this._navTitle.isEmpty
            ? Settings.locale((s) => s.contacts, ctx: context)
            : this._navTitle,
        actions: [
          IconButton(
            icon: Asset.iconSvg(
                    ...
                    Expanded(
                      child: FixedTextField(
                        ...
                        decoration: InputDecoration(
                          hintText:
                              Settings.locale((s) => s.search, ctx: context),
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  int groupItemIndex =
                      index - searchTopicViewCount - searchFriendViewCount - 1;
                  /*int strangerItemIndex = index - searchGroupViewCount - searchTopicViewCount - searchFriendViewCount - 1;*/

                  if (searchFriendViewCount > 0 &&
                      index >= friendStartIndex &&
                      index <= friendEndIndex) {
                  ...
                  } else if (searchTopicViewCount > 0 &&
                      index >= topicStartIndex &&
                      index <= topicEndIndex) {
                  ...
                  } else if (searchGroupViewCount > 0 &&
      ...
      child: Column(
        ...
        children: [
          ContactItem(
            ...
            bodyDesc: Time.formatTime(
                DateTime.fromMillisecondsSinceEpoch(item.updateAt)),
          ...
          CustomSlidableAction(
            onPressed: (BuildContext context) {
              ModalDialog.of(Settings.appContext).confirm(
                title: Settings.locale((s) => s.delete_contact_confirm_title,
                    ctx: context),
                ...
                agree: Button(
                  ...
                  onPressed: () async {
                    ...
                    await contactCommon.setType(item.address, ContactType.none,
                        notify: true);
                  },
                ),
                reject: Button(
      ...
      child: Column(
        ...
        children: [
          TopicItem(
            ...
            bgColor: Colors.transparent,
            bodyTitle: item.displayNameShort,
      ...
      endActionPane: ActionPane(
        ...
        children: [
          CustomSlidableAction(
            onPressed: (BuildContext context) {
              ModalDialog.of(Settings.appContext).confirm(
                title: Settings.locale((s) => s.confirm_unsubscribe_group,
                    ctx: context),
                ...
                agree: Button(
                  ...
                  onPressed: () async {
                    if (Navigator.of(this.context).canPop())
                      Navigator.pop(this.context);
                    double? fee =
                        await topicCommon.getTopicSubscribeFee(this.context);
                    if (fee == null) return;
                    Loading.show();
                    TopicSchema? deleted = await topicCommon
                        .unsubscribe(item.topicId, fee: fee, toast: true);
                    Loading.dismiss();
                    if (deleted != null) {
                      Toast.show(
                          Settings.locale((s) => s.unsubscribed, ctx: context));
                    }
                  },
                ),
                reject: Button(
                ...
                agree: Button(
                  ...
                  onPressed: () async {
                    if (Navigator.of(this.context).canPop())
                      Navigator.pop(this.context);
                    Loading.show();
                    bool success = await privateGroupCommon.quit(item.groupId,
                        toast: true, notify: true);
                    Loading.dismiss();
                    if (success)
                      Toast.show(
                          Settings.locale((s) => s.unsubscribed, ctx: context));
