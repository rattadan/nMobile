...
import 'package:nchat_mobile/components/entropy/touch_entropy_collector.dart';
...
class _FirstWelcomeScreenState
  ...
  void _onEntropyCollected(String entropy) {
    ...
    // Set initial mnemonic/seed state
    setState(() {
    ...
    // Ensure WalletService knows the mnemonic immediately so EVM address can
    // be derived on the first seed screen render, without requiring edits.
    try {
      _walletService.setMnemonic(_mnemonic);
    } catch (e) {
      logger.e('Error setting mnemonic on WalletService: $e');
    }
  ...
  void _validateAndSetCustomMnemonic() {
    ...
    setState(() {
      ...
      _words = _mnemonic.split(' '); // Initialize words list for editing
      _updateSeedFromMnemonic(); // Update seed from new mnemonic
  ...
  // Derive seed from mnemonic whenever mnemonic changes
  void _updateSeedFromMnemonic() {
    try {
      // Convert mnemonic back to entropy
      final entropyHex = bip39.mnemonicToEntropy(_mnemonic);
      final entropyBytes = hexDecode(entropyHex);

      // Derive 32-byte seed from entropy (same logic as initial creation)
      final seed32 = crypto.sha256.convert(entropyBytes).bytes;
      _seedHex32 =
          seed32.map((b) => b.toRadixString(16).padLeft(2, '0')).join('');

      // Update wallet service with new mnemonic for ETH address derivation
      _walletService.setMnemonic(_mnemonic);
    } catch (e) {
      logger.e('Error deriving seed from mnemonic: $e');
      // If derivation fails, keep existing seed or clear it
    }
  }
  ...
  void _editWord(int index) async {
    ...
    if (selected != null && selected != currentWord) {
      ...
      // Update seed from new mnemonic to refresh derived addresses
      _updateSeedFromMnemonic();
      ...
      body: SingleChildScrollView(
        child: Padding(
          padding: const EdgeInsets.fromLTRB(
              20, 20, 20, 80), // Added bottom safe space
          child: Form(
            key: _formKey,
            onChanged: () {
              setState(() {
          ...
          child: Column(
            ...
            children: [
              ...
              FutureBuilder<String>(
                future: _walletService.getEthereumAddress(),
                builder: (context, snapshot) {
                  final address = snapshot.data ?? 'Deriving...';
                  return Row(
                    children: [
                      Expanded(
                        child: Text(
        ...
        onPressed: () {
          ...
          setState(() {
            if (_pinController.text.length < 4) {
              ...
              // Check if PIN entry is complete and valid
              if (_pinConfirmController.text.length == 4) {
                _formValid = _validateForm();
                if (_formValid && _entropyCollected) {
                  // Auto-trigger wallet creation when PIN is valid
                  _createWallet();
                }
                return; // Skip the setState below since _createWallet handles it
              }
            }
            _formValid = _validateForm();
          });
