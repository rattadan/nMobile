...
class WalletBloc extends Bloc<WalletEvent, WalletState> {
  ...
  Stream<WalletState> _mapAddWalletToState(AddWallet event) async* {
    await _walletStorage.add(
        event.wallet, event.keystore, event.password, event.seed,
        mnemonic: event.mnemonic);
    List<WalletSchema> list = List.from((state as WalletLoaded).wallets);
    int index = list.indexWhere((x) =>
        x.address == event.wallet.address); // int index = list.indexOf(wallet);
    if (index >= 0) {
  ...
  Stream<WalletState> _mapDeleteWalletToState(DeleteWallet event) async* {
    ...
    int index = list.indexWhere(
        (w) => w.address == deleteAddress); // int index = list.indexOf(wallet);
    if (index >= 0) {
      list.removeAt(index);
  ...
  Stream<WalletState> _mapUpdateWalletToState(UpdateWallet event) async* {
    ...
    int index = list.indexWhere((w) =>
        w.address == wallet.address); // int index = list.indexOf(wallet);
    if (index >= 0) {
      list[index] = wallet;
  ...
  Stream<WalletState> _mapBackupWalletToState(BackupWallet event) async* {
    ...
    final List<WalletSchema> list = List.from((state as WalletLoaded).wallets);
    int index = list.indexWhere(
        (w) => w.address == backupAddress); // int index = list.indexOf(wallet);
    if (index >= 0) {
