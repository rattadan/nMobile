...
import 'package:nchat_mobile/components/image/image_cropper_page.dart';
...
class MediaPicker {
  static Future<List<Map<String, dynamic>>> pickCommons(
    ...
    for (var i = 0; i < pickedResults.length; i++) {
      ...
      if (savePath.isNotEmpty) {
        ...
        if (isAudio || isVideo) {
          params.addAll({"duration": null});
        }
        pickedMaps.add(params);
      }
      logger
          .i("MediaPicker - pickCommons - picked success - params:$pickedMaps");
    }
    return pickedMaps;
  }

  static Future<Map<String, dynamic>?> takeImage(String? savePath,
    ...
    XFile? pickedResult;
    try {
      pickedResult = await ImagePicker().pickImage(
          source: ImageSource
              .camera); // imageQuality: compressQuality  -> ios no enable
    } catch (e, st) {
      handleError(e, st);
    }
    ...
    if (savePath.isNotEmpty) {
      ...
      logger.i(
          "MediaPicker - takeImage - picked success - params:${params.toString()}");
      return params;
    }
  ...
  static Future<File?> pickImage({
    bool? cropRectangle, // true = rectangle, false = circle
    int? maxSize,
    int? bestSize,
    String? savePath,
  }) async {
    ...
    pickedFile.length().then((value) {
      logger.i(
          'MediaPicker - pickImage - picked - size:${Format.flowSize(value.toDouble(), unitArr: [
            'B',
            'KB',
            'MB',
            'GB'
    ...
    // Preprocess large images before crop to improve cropper UX/perf
    // Target: ~320px minimum dimension (keeps aspect ratio)
    if (cropRectangle != null) {
      try {
        final fileExt =
            Path.getFileExt(pickedFile, FileHelper.DEFAULT_IMAGE_EXT);
        final preprocessPath = await Path.getRandomFile(
          null,
          DirType.cache,
          fileExt: fileExt,
        );
        ...
        if (preprocessPath.toLowerCase().endsWith(".jpg") ||
            preprocessPath.toLowerCase().endsWith(".jpeg")) {
          format = CompressFormat.jpeg;
        } else if (preprocessPath.toLowerCase().endsWith(".png")) {
          format = CompressFormat.png;
        } else if (preprocessPath.toLowerCase().endsWith(".heic")) {
          format = CompressFormat.heic;
        } else if (preprocessPath.toLowerCase().endsWith(".webp")) {
          format = CompressFormat.webp;
        }

        if (format != null) {
          final preFile = await FlutterImageCompress.compressAndGetFile(
    ...
    // crop
    pickedFile = await _cropImage(pickedFile,
        cropRectangle: cropRectangle, savePath: savePath);
    if (pickedFile == null) {
    ...
    // compress
    pickedFile = await compressImageBySize(pickedFile,
        maxSize: maxSize ?? 0, bestSize: bestSize ?? 0, toast: true);
    if (pickedFile == null) {
    ...
    if (savePath == null || savePath.isEmpty) {
      savePath =
          await Path.getRandomFile(null, DirType.cache, fileExt: fileExt);
    } else {
  ...
  static Future<File?> _cropImage(File? original,
      {bool? cropRectangle, String? savePath}) async {
    if (original == null) return null;
    if (cropRectangle == null) return original;
    // gif
    ...
    File? cropFile;
    try {
      // Use custom Material Design cropper
      // Navigate to cropper page
      cropFile = await Navigator.of(Settings.appContext).push(
        MaterialPageRoute<File>(
          builder: (context) => ImageCropperPage(
            imageFile: original,
            savePath: savePath,
            isRectangle: cropRectangle,
            quality: 80,
          ),
        ),
      );
    ...
    if (cropFile != null) {
      int size = await cropFile.length();
      logger.i(
          'MediaPicker - _cropImage - size:${Format.flowSize(size.toDouble(), unitArr: [
            'B',
            'KB',
            'MB',
            'GB'
  ...
  static Future<File?> compressImageBySize(File? original,
      {String? savePath,
      int maxSize = 0,
      int bestSize = 0,
      bool force = false,
    ...
    if (!maxEnable && !bestEnable) {
      logger.i(
          'MediaPicker - _compressImage - no compress - originalSize:${Format.flowSize(originalSize.toDouble(), unitArr: [
            'B',
            'KB',
            'MB',
            'GB'
      ...
      if (originalSize <= maxSize) {
        logger.i(
            'MediaPicker - _compressImage - ok with only maxSize - originalSize:${Format.flowSize(originalSize.toDouble(), unitArr: [
              'B',
              'KB',
              'MB',
              'GB'
            ])} - maxSize:${Format.flowSize(maxSize.toDouble(), unitArr: [
              'B',
              'KB',
              'MB',
              'GB'
      ...
      if (originalSize <= bestSize) {
        logger.i(
            'MediaPicker - _compressImage - ok with only bestSize - originalSize:${Format.flowSize(originalSize.toDouble(), unitArr: [
              'B',
              'KB',
              'MB',
              'GB'
            ])} - bestSize:${Format.flowSize(bestSize.toDouble(), unitArr: [
              'B',
              'KB',
              'MB',
              'GB'
      ...
      if ((originalSize <= bestSize) && (originalSize <= maxSize)) {
        logger.i(
            'MediaPicker - _compressImage - ok with size - originalSize:${Format.flowSize(originalSize.toDouble(), unitArr: [
              'B',
              'KB',
              'MB',
              'GB'
    ...
    logger.i(
        'MediaPicker - _compressImage - compress:START - originalSize:${Format.flowSize(originalSize.toDouble(), unitArr: [
          'B',
          'KB',
          'MB',
          'GB'
    ...
    String compressPath = savePath ??
        (await Path.getRandomFile(null, DirType.cache, fileExt: fileExt));
    // format
    CompressFormat? format;
    if (compressPath.toLowerCase().endsWith(".jpg") ||
        compressPath.toLowerCase().endsWith(".jpeg")) {
      format = CompressFormat.jpeg;
    ...
    if (format == null) {
      logger.e(
          'MediaPicker - _compressImage - compress:FAIL - CompressFormatError - compressPath:$compressPath');
      return original;
    }
    ...
    try {
      ...
      for (var i = 0; i < qualityItems.length; i++) {
        ...
        logger.d(
            'MediaPicker - _compressImage - compress:OK - tryTimes:${i + 1} - quality:$compressQuality - compressSize:${Format.flowSize(compressSize.toDouble(), unitArr: [
              'B',
              'KB',
              'MB',
              'GB'
            ])} - originalSize:${Format.flowSize(originalSize.toDouble(), unitArr: [
              'B',
              'KB',
              'MB',
              'GB'
    ...
    if (compressSize > maxSize) {
      logger.w(
          'MediaPicker - _compressImage - compress:BREAK - compressSize:${Format.flowSize(compressSize.toDouble(), unitArr: [
            'B',
            'KB',
            'MB',
            'GB'
            ...
            'MB',
            'GB'
          ])} - format:$format - path:${compressFile?.path}');
      if (toast) Toast.show(Settings.locale((s) => s.file_too_big));
      return null;
    }
    logger.i(
        'MediaPicker - _compressImage - compress:END - compressSize:${Format.flowSize(compressSize.toDouble(), unitArr: [
          'B',
          'KB',
          'MB',
          'GB'
  ...
  static Future<Map<String, dynamic>?> getVideoThumbnail(
      String filePath, String savePath,
      {int quality = 20, int maxWidth = 100}) async {
    if (filePath.isEmpty || savePath.isEmpty) return null;
    logger.w(
        'MediaPicker - getVideoThumbnail - video thumbnails disabled - filePath:$filePath');
    return null;
  }
