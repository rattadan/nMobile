...
class CustomFilter extends LogFilter {
  ...
  bool shouldLog(LogEvent event) {
    ...
    bool levelOk =
        isVerbose || isDebug || isInfo || isWarning || isError || isWtf;
    return levelOk;
  }
}
...
mixin Tag {
  ...
  String _tagInner(int length, int lenHashCode) {
    ...
    int endIndex =
        (hashCodeStr.length > lenHashCode) ? lenHashCode : hashCodeStr.length;
    final String name =
        this.runtimeType.toString() + '@' + hashCodeStr.substring(0, endIndex);
    return name;
  }
}
...
class ColorPrinter extends LogPrinter {
  ...
  List<String> log(LogEvent event) {
    ...
    var timeStr = printTime
        ? color('TIME: ${data.minute}:${data.second}:${data.millisecond}')
        : '';
