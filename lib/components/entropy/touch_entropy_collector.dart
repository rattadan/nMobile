...
class _TouchEntropyCollectorState extends State<TouchEntropyCollector> {
        ...
        Container(
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              // Store the container size for boundary checking
  ...
  void _handlePanStart(DragStartDetails details) {
    ...
    if (!_isPointInBounds(details.localPosition)) return;
  ...
  bool _isPointInBounds(Offset point) {
    if (_containerSize == null) return true; // Allow until size is known
    return point.dx >= 0 &&
        point.dx <= _containerSize!.width &&
        point.dy >= 0 &&
        point.dy <= _containerSize!.height;
  }
  ...
  void _addPoint(Offset point) {
    setState(() {
      ...
      final byEntropy = (_entropyData.length * estimatedEntropy * 8) /
          widget.requiredEntropyBits;

      // Calculate time-based progress
      final nowMs = DateTime.now().millisecondsSinceEpoch;
      final byTime = widget.minDurationMs <= 0
          ? 1.0
          : (nowMs - (_startMs ?? nowMs)) / widget.minDurationMs;
