...
import 'package:nchat_mobile/components/chat/bubble.dart';
...
class _ChatMessageItemState extends BaseStateFulWidgetState<ChatMessageItem> {
  ...
  void initState() {
    super.initState();
    // contact
    _contactUpdateStreamSubscription = contactCommon.updateStream
        .where((event) => event.address == _sender?.address)
        .listen((event) {
  ...
  void _refreshSender() {
    if ((_sender?.address.isNotEmpty == true) &&
        (_sender?.address == widget.message.sender)) return;
    if (widget.message.temp?["sender"] == null) {
      ...
      contactCommon.query(widget.message.sender).then((sender) {
        if (widget.message.sender == sender?.address) {
          ...
          sender =
              ContactSchema.create(widget.message.sender, ContactType.none);
          setState(() {
            _sender = sender;
          });
        }
  ...
  /// Load avatar from cache for topic/group messages
  Future<void> _loadAvatarFromCache() async {
    try {
      // Only for inbound messages (received from others)
      if (widget.message.isOutbound) return;

      final sender = _sender;
      if (sender == null) return;

      File? avatarFile;
      AvatarSenderType? senderType;

      // Determine sender type and load avatar
      if (widget.message.isTargetTopic) {
        // Topic (public group) message - try cache first
        senderType = AvatarSenderType.topic;
        final topicId = widget.message.targetId;
        avatarFile = await AvatarCacheService().getAvatar(topicId, senderType);
      } else if (widget.message.isTargetGroup) {
        // Private group message - try cache first
        senderType = AvatarSenderType.privateGroup;
        final groupId = widget.message.targetId;
        avatarFile = await AvatarCacheService().getAvatar(groupId, senderType);
      } else if (widget.message.isTargetContact) {
        // Contact message - use contact avatar
        avatarFile = await sender.displayAvatarFile;
      }
      ...
      // Ignore errors, will fallback to default avatar
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // isOutbound
    bool sameReceiveBot = (widget.message.isOutbound == false) &&
        (widget.message.isOutbound == widget.prevMessage?.isOutbound);
    bool sameReceiveTop = (widget.message.isOutbound == false) &&
        (widget.message.isOutbound == widget.nextMessage?.isOutbound);
    // status
    bool sameStatusBot = (sameUserBot && sameReceiveBot)
        ? true
        : widget.message.status == widget.prevMessage?.status;
    bool sameStatusTop = (sameUserTop && sameReceiveTop)
        ? true
        : widget.message.status == widget.nextMessage?.status;
    // type
    ...
    // profile
    bool leftBigMargin = visibleSelf &&
        !widget.message.isOutbound &&
        (widget.message.isTargetTopic || widget.message.isTargetGroup);
    ...
    switch (this.widget.message.contentType) {
      ...
      case MessageContentType.reaction:
        // Reaction messages are broadcast-only and should not be displayed
        return SizedBox.shrink();
    }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          isSendOut
              ? SizedBox.shrink()
              : _widgetBubbleAvatar(avatarVisible, avatarTrans),
          SizedBox(width: 8),
          Expanded(
            child: Column(
              ...
              crossAxisAlignment:
                  isSendOut ? CrossAxisAlignment.end : CrossAxisAlignment.start,
              children: [
                ...
                ChatBubble(
                  ...
                  reactionSummary: this.widget.reactionSummary,
                  onReactionTap: this.widget.onReactionTap,
                ),
    ...
    return visible
        ? Opacity(
            ...
            child: GestureDetector(
              onTap: () async {
                if (!trans)
                  this.widget.onAvatarPress?.call(_sender!, widget.message);
              },
              onLongPress: () {
                if (!trans)
                  this.widget.onAvatarLonePress?.call(_sender!, widget.message);
              },
              child: (_sender != null)
                  ? _buildAvatarWidget()
  ...
  /// Build avatar widget with cache support
  Widget _buildAvatarWidget() {
    // Use cached avatar file if available
    if (_avatarFile != null) {
      return CircleAvatar(
        radius: 20,
        backgroundImage: FileImage(_avatarFile!),
        onBackgroundImageError: (e, stackTrace) {
          // Fallback to ContactAvatar if cached file fails
          setState(() {
            _avatarFile = null;
          });
        },
  ...
  Widget _widgetContactOptions(BuildContext context) {
    Map<String, dynamic> optionData =
        this.widget.message.content ?? Map<String, dynamic>();
  ...
  Widget _widgetTopicSubscribe(BuildContext context) {
    String who = widget.message.isOutbound
        ? Settings.locale((s) => s.you, ctx: context)
        : _sender?.displayName ?? widget.message.sender.substring(0, 6);
    String content =
        who + ' ' + Settings.locale((s) => s.joined_channel, ctx: context);
  ...
  Widget _widgetTopicInvited(BuildContext context) {
    String receiver = (widget.message.targetId.length > 6)
        ? widget.message.targetId.substring(0, 6)
        : " ";
    String sender = widget.message.sender.length > 6
        ? widget.message.sender.substring(0, 6)
        : " ";
    String inviteDesc = widget.message.isOutbound
        ? Settings.locale((s) => s.invites_desc_other(receiver), ctx: context)
        : Settings.locale((s) => s.invites_desc_me(sender), ctx: context);
        ...
        children: <Widget>[
          Row(
            ...
            children: [
              ...
              SizedBox(width: 2),
              Label(
                TopicSchema(topicId: widget.message.content?.toString() ?? " ")
                    .topicNameShort,
                maxWidth: Settings.screenWidth() * 0.5,
                type: LabelType.bodyRegular,
                fontWeight: FontWeight.bold,
          ...
          widget.message.isOutbound
              ...
              : InkWell(
                  ...
                  onTap: () async {
                    String? topic =
                        await BottomDialog.of(Settings.appContext).showInput(
                      title: Settings.locale((s) => s.accept_invitation,
                          ctx: context),
                      desc: inviteDesc,
                      value: widget.message.content?.toString() ?? " ",
                      actionText: Settings.locale((s) => s.accept_invitation,
                          ctx: context),
                      enable: false,
                    );
                    if (topic?.isNotEmpty == true) {
                      double? fee = await topicCommon
                          .getTopicSubscribeFee(Settings.appContext);
                      if (fee == null) return;
                      Loading.show();
                      bool isJustNow = (DateTime.now().millisecondsSinceEpoch -
                              widget.message.sendAt) <
                          Settings.gapTxPoolUpdateDelayMs;
                      TopicSchema? result = await topicCommon.subscribe(topic,
                          fetchSubscribers: true, justNow: isJustNow, fee: fee);
                      Loading.dismiss();
                      if (result != null)
                        Toast.show(
                            Settings.locale((s) => s.subscribed, ctx: context));
                    }
                  },
                )
        ],
  ...
  Widget _widgetPrivateGroupInvited(BuildContext context) {
    ...
    Map content = (widget.message.content != null)
        ? (widget.message.content as Map)
        : Map();
    String groupId = content['groupId']?.toString() ?? "";
    String groupName = content['name']?.toString() ?? "";
    int type = int.tryParse(content['type']?.toString() ?? "0") ??
        PrivateGroupType.normal;
    ...
    if (expiresAt < DateTime.now().millisecondsSinceEpoch) {
      ...
      expiresWidget = widget.message.isOutbound
          ? Label(
              Settings.locale((s) => s.expiration, ctx: context) +
                  ': ' +
                  Time.formatTimeFromNow(
                ...
                InkWell(
                  ...
                  onTap: () async {
                    ...
                    if (value?.isNotEmpty == true) {
                      ...
                      PrivateGroupItemSchema? groupItemSchema =
                          PrivateGroupItemSchema.fromRawData(itemData);
                      groupItemSchema = await privateGroupCommon
                          .acceptInvitation(groupItemSchema, toast: true);
                      if (groupItemSchema != null) {
                        if (await chatOutCommon.sendPrivateGroupAccept(
                            inviter, groupItemSchema)) {
                          groupSchema = PrivateGroupSchema.create(
                              groupId, groupName,
                              type: type);
                          if (groupSchema != null) {
                            groupSchema = await privateGroupCommon
                                .addPrivateGroup(groupSchema, notify: true);
                          }
                          if (groupSchema != null) {
                            await sessionCommon.add(
                                groupSchema.groupId, SessionType.PRIVATE_GROUP,
                                lastMsgAt:
  ...
  Widget _widgetPrivateGroupSubscribe(BuildContext context) {
    ...
    String inviteeDisplay =
        (invitee.length > 6) ? invitee.substring(0, 6) : invitee;
    String who = ((clientCommon.address == invitee)
        ? Settings.locale((s) => s.you, ctx: context)
        : "$inviteeDisplay: ");
    String content =
        who + Settings.locale((s) => s.joined_channel, ctx: context);
