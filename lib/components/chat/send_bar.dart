...
class _ChatSendBarState extends BaseStateFulWidgetState<ChatSendBar> {
  ...
  ColorTween _audioBgColorTween =
      ColorTween(begin: Colors.transparent, end: Colors.red);
  ColorTween _audioTextColorTween =
  ...
  void initState() {
    ...
    // record
    _onRecordProgressSubscription =
        audioHelper.onRecordProgressStream.listen((event) {
      ...
      if ((_audioRecordDurationMs < Settings.durationAudioRecordMaxS * 1000) &&
          (duration.inMilliseconds >=
              Settings.durationAudioRecordMaxS * 1000)) {
  ...
  _onGesture(
    ...
    if (0 <= offsetY && offsetY <= ActionHeight) {
      ...
      } else if (ActionWidth < offsetX &&
          offsetX < (Settings.screenWidth() - ActionWidth)) {
        // center
        if (!_audioRecordVisible) {
          // text editing
        } else {
          ...
          if (!isMove) {
            ...
            if ((offsetX - ActionWidth) > (touchWidth / 2)) {
              percent = 1 -
                  (((touchWidth / 2) - (offsetX - ActionWidth)) /
                          (touchWidth / 2))
                      .abs();
            }
      ...
      } else if ((Settings.screenWidth() - ActionWidth) <= offsetX &&
          offsetX <= Settings.screenWidth()) {
        // right
        if (_canSendText) {
          // text send
          if (!isMove) {
    ...
    } else if (-(ChatSendBar.LockActionMargin + ChatSendBar.LockActionSize) <=
            offsetY &&
        offsetY <= -ChatSendBar.LockActionMargin) {
      if (!_audioRecordVisible) {
        // nothing
      } else {
        if ((Settings.screenWidth() -
                    ChatSendBar.LockActionMargin -
                    ChatSendBar.LockActionSize) <=
                offsetX &&
            offsetX <=
                (Settings.screenWidth() - ChatSendBar.LockActionMargin)) {
          // lock mode
          if (!isMove) {
  ...
  _setAudioRecordVisible(bool visible, bool complete) async {
    ...
    var result = await widget.onRecordTap
        ?.call(visible, complete, _audioRecordDurationMs);
  ...
  Widget build(BuildContext context) {
    ...
    Color? recordWidgetColor =
        _audioTextColorTween.transform(_audioDragPercent);
    Color? volumeColor = (_audioRecordDurationMs % 1000) <= 500
        ? recordWidgetColor
        : Colors.transparent;

    Duration recordDuration = Duration(milliseconds: _audioRecordDurationMs);
    String recordMin =
        "${recordDuration.inMinutes < 10 ? 0 : ""}${recordDuration.inMinutes}";
    String recordSec =
        "${recordDuration.inSeconds < 10 ? 0 : ""}${recordDuration.inSeconds}";
    ...
    return GestureDetector(
      onTapDown: (TapDownDetails details) {
        this._onGesture(
          ...
          // Removed: menuToggle - swiper always visible now
            ...
            Expanded(
              child: Stack(
                children: [
                  Row(
                    children: [
                      // Grid button removed - swiper always visible now
                      Expanded(
                        child: Container(
                              ...
                              Expanded(
                                child: FixedTextField(
                                  style: TextStyle(
                                      ...
                                      height: 1.4,
                                      color: Color(
                                          0xFF001F3F)), // very dark blue text
                                  decoration: InputDecoration(
                                    hintText: Settings.locale(
                                        (s) => s.type_a_message,
                                        ctx: context),
                                    hintStyle:
                                        TextStyle(color: _theme.fontColor2),
                                  ...
                                  onChanged: (val) {
                                    ...
                                    if (draft.isNotEmpty) {
                                      memoryCache.setDraft(
                                          widget.targetId, draft);
                                    } else {
                                      memoryCache.removeDraft(widget.targetId);
                                    }
                                    setState(() {
                  ...
                  _audioRecordVisible
                      ? Container(
                          ...
                          child: Container(
                            color:
                                _audioBgColorTween.transform(_audioDragPercent),
                            child: Row(
                              children: [
                                SizedBox(width: 16),
                                Icon(FontAwesomeIcons.microphone,
                                    size: 24, color: volumeColor),
                                SizedBox(width: 8),
                                Container(
                                  child: Label(
                                    recordDurationText,
                                ...
                                Expanded(
                                  child: Container(
                                    ...
                                    child: Center(
                                      child: Label(
                                        _audioLockedMode
                                            ? Settings.locale((s) => s.cancel,
                                                ctx: context)
                                            : (_audioDragPercent != 0
                                                ? Settings.locale(
                                                    (s) => s.release_to_cancel,
                                                    ctx: context)
                                                : Settings.locale(
                                                    (s) => s.slide_to_cancel)),
