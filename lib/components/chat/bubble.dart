...
import 'package:nchat_mobile/components/button/button_icon.dart';
...
import 'package:nchat_mobile/components/text/markdown.dart';
import 'package:nchat_mobile/components/tip/popup_menu.dart' as PopMenu;
...
class _ChatBubbleState extends BaseStateFulWidgetState<ChatBubble> with Tag {
  ...
  void onRefreshArguments() {
    ...
    _message =
        chatCommon.burningTick(_message, true, onTick: () => setState(() {}));
    // progress
    _refreshProgress();
    // thumbnail
  ...
  void initState() {
    super.initState();
    // progress
    _onProgressStreamSubscription =
        messageCommon.onProgressStream.listen((Map<String, dynamic> event) {
    ...
    // player
    _onPlayProgressSubscription =
        audioHelper.onPlayProgressStream.listen((Map<String, dynamic> event) {
  ...
  void _refreshProgress() {
    ...
    double fetchProgress =
        double.tryParse(temp?["mediaFetchProgress"]?.toString() ?? "0") ?? 0;
    if ((_fetchProgress < 0) ||
        (_fetchProgress > 1) ||
        (_fetchProgress != fetchProgress)) {
      _fetchProgress =
          (_message.status == MessageStatus.Sending) ? fetchProgress : -1;
    }
    // playProgress
    double playProgress =
        double.tryParse(temp?["mediaPlayProgress"]?.toString() ?? "0") ?? 0;
    if ((_playProgress < 0) ||
        (_playProgress > 1) ||
  ...
  void _refreshMediaThumbnail() {
    if (_message.temp == null) _message.temp = Map();
    Map? temp = _message.temp;
    if ((_thumbnailPath?.isNotEmpty == true) &&
        (_thumbnailPath == temp?["existsMediaThumbnailPath"])) return;
  ...
  Widget build(BuildContext context) {
    ...
    bool isTipBottom = (_message.status == MessageStatus.Success) ||
        (_message.status == MessageStatus.Receipt);

    var widget = Row(
      mainAxisSize: MainAxisSize.max,
      mainAxisAlignment:
          isSendOut ? MainAxisAlignment.end : MainAxisAlignment.start,
      crossAxisAlignment:
          isTipBottom ? CrossAxisAlignment.end : CrossAxisAlignment.center,
      children: [
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Reactions bar
            if (widget.reactionSummary != null)
              MessageReactionsBar(
                reactionSummary: widget.reactionSummary!,
                onTap: widget.onReactionTap,
                isMe: _message.isOutbound,
              ),
          ],
        ),
      ),
    ...
    switch (contentType) {
      ...
      case MessageContentType.textExtension:
        onTap = () {
          ...
          // Always add reaction option if onReactionTap is available
          if (widget.onReactionTap != null) {
            menuItems.add(PopMenu.MenuItem(
              userInfo: 0,
              title: "React",
              textStyle: TextStyle(
                  color: application.theme.fontLightColor, fontSize: 12),
            ));
          }
      ...
      case MessageContentType.image:
        ...
        bool fileNative = _message.isOutbound ||
            (_message.contentType != MessageContentType.ipfs);
        bool fileDownloaded =
            (_message.contentType == MessageContentType.ipfs) &&
                (state == MessageOptions.ipfsStateYes);

        if (_message.isContentFile && (fileNative || fileDownloaded)) {
          File file = _message.content as File;
          Map<String, dynamic>? item = MediaScreen.createMediasItemByImagePath(
              _message.msgId, file.path);
          if (item != null) {
            onTap = () {
              // Show menu with reaction and view options
              List<PopMenu.MenuItem> menuItems = [];
                  ...
                  switch (index) {
                    ...
                    case 1: // View
                      Map<String, dynamic>? mediaItem =
                          MediaScreen.createMediasItemByImagePath(
                              _message.msgId, file.path);
                      if (mediaItem != null) {
                        MediaScreen.go(context, [mediaItem],
                            target: _message.targetId,
                            leftMsgId: _message.msgId);
                      }
                      break;
                  }
                },
      ...
      case MessageContentType.audio:
        ...
        if (_message.isContentFile) {
          ...
          int? durationMs =
              (durationS == null) ? null : ((durationS * 1000).round());
          File file = _message.content as File;
          onTap = () {
            audioHelper.playStart(_message.msgId, file.path,
      ...
      case MessageContentType.video:
        ...
        if (_message.isContentFile && (fileNative || fileDownloaded)) {
          File file = _message.content as File;
          Map<String, dynamic>? item = MediaScreen.createMediasItemByVideoPath(
              _message.msgId, file.path, _thumbnailPath);
          if (item != null) {
            onTap = () {
              MediaScreen.go(context, [item],
                  target: _message.targetId, leftMsgId: _message.msgId);
            };
          }
        } else if (state == MessageOptions.ipfsStateNo) {
          onTap = () {
  ...
  Widget _widgetBubbleInfoBottom() {
    Color color = _message.isOutbound
        ? application.theme.fontLightColor.withAlpha(178)
        : application.theme.fontColor2.withAlpha(178);

    String sendTime = Time.formatChatTime(
        DateTime.fromMillisecondsSinceEpoch(_message.sendAt));
    bool isSending = _message.status == MessageStatus.Sending;

    bool showTime = isSending || widget.showTimeAndStatus;
    bool showBurn = _message.canBurning &&
        (_message.deleteAt != null) &&
        (_message.deleteAt != 0);
    bool showStatus =
        (isSending || widget.showTimeAndStatus) && _message.isOutbound;
        ...
        ? Row(
            ...
            children: [
              (showTime || showBurn)
                  ...
                  : SizedBox.shrink(),
              ((showTime || showBurn) && showBurn)
                  ? SizedBox(width: 4)
                  : SizedBox.shrink(),
              showBurn ? _widgetBurning() : SizedBox.shrink(),
              ((showTime || showBurn) && showStatus)
                  ? SizedBox(width: 4)
                  : SizedBox.shrink(),
  ...
  Widget _widgetBurning() {
    ...
    int? burnAfterSeconds =
        MessageOptions.getOptionsBurningDeleteSec(_message.options);
  ...
  Widget _widgetSendingInfo() {
    ...
    bool showProgress = isSending &&
        _message.isContentFile &&
        (_fetchProgress < 1) &&
        (_fetchProgress > 0);
  ...
  List<Widget> _widgetBubbleImage() {
    double iconSize =
        min(Settings.screenWidth() * 0.1, Settings.screenHeight() * 0.06);
    ...
    if ((_message.isOutbound == false) &&
      ...
      String? fileSize = _size > 0
          ? Format.flowSize(_size.toDouble(),
              unitArr: ['B', 'KB', 'MB', 'GB'], decimalDigits: 0)
          : null;
      int state = MessageOptions.getIpfsState(_message.options);
                      ...
                      child: Container(
                        ...
                        child: showProgress
                            ? CircularProgressIndicator(
                                backgroundColor:
                                    application.theme.fontColor4.withAlpha(80),
                                color: Colors.white,
                                strokeWidth: 3,
                                value: _fetchProgress,
                              )
  ...
  List<Widget> _widgetBubbleAudio() {
    ...
    Color iconColor = _message.isOutbound
        ? Colors.white.withAlpha(200)
        : application.theme.primaryColor.withAlpha(200);
    Color textColor = _message.isOutbound
        ? Colors.white.withAlpha(200)
        : application.theme.fontColor2.withAlpha(200);
    Color progressBgColor = _message.isOutbound
        ? Colors.white.withAlpha(230)
        : application.theme.primaryColor.withAlpha(30);
    Color progressValueColor = _message.isOutbound
        ? application.theme.backgroundColor4.withAlpha(127)
        : application.theme.primaryColor.withAlpha(200);

    double? durationS = MessageOptions.getMediaDuration(_message.options);
    double maxDurationS = Settings.durationAudioRecordMaxS;
    double durationRatio = ((durationS ?? (maxDurationS / 2)) > maxDurationS
            ? maxDurationS
            : (durationS ?? (maxDurationS / 2))) /
        maxDurationS;
    ...
    return [
      Row(
        ...
        children: [
          ...
          SizedBox(width: 8),
          Label('$durationText\"',
              type: LabelType.bodyRegular, color: textColor),
        ],
      ),
    ];
  }
          ...
          children: [
            Column(
              ...
              children: [
                ConstrainedBox(
                  constraints: BoxConstraints(
                      maxWidth: labelWidth, minWidth: labelWidth),
                  child: Label(
                    fileName,
  ...
  List<double?> _getPlaceholderWH(List<double> maxWH, List<double> realWH) {
    ...
    if (realWH.length < 2 || realWH[0] <= 0 || realWH[1] <= 0)
      return [null, null];
      ...
      decoration = BoxDecoration(
        ...
        borderRadius: BorderRadius.only(
          ...
          bottomRight: Radius.circular(
              widget.hideBotMargin ? 1 : (widget.hideTopMargin ? 12 : 2)),
        ),
      );
    } else {
      decoration = BoxDecoration(
        ...
        borderRadius: BorderRadius.only(
          topLeft: Radius.circular(
              widget.hideTopMargin ? 1 : (widget.hideBotMargin ? 12 : 2)),
  ...
  _widgetMessageDebugInfo() {
    ...
    String queueID = (_message.queueId > 0)
        ...
        : "";
    String _firstQueueIDs =
        MessageOptions.getFirstMessageQueueIds(_message.options) ?? "";
    String firstQueueIDs = _firstQueueIDs.isNotEmpty
        ? "\nsync● ${(_firstQueueIDs.length > 25) ? _firstQueueIDs.substring(0, 24) : _firstQueueIDs}"
        : "";
    String _queueIds =
        MessageOptions.getMessageQueueIds(_message.options) ?? "";
    ...
    String resendMuteAt = (_resendOk > 0)
        ...
        : "";
    String _pushNotifyId =
        MessageOptions.getPushNotifyId(_message.options) ?? "";
