...
import 'package:nchat_mobile/generated/l10n.dart';
...
class Settings {
  ...
  static bool discoveryBroadcastEnabled = true; // enabled/disabled
  static int discoveryBroadcastPeriod = 1; // minutes

  // profile setup
  static bool profileSetupCompleted = false;
  ...
  static late Directory
      applicationRootDirectory; // eg:/data/user/0/org.nkn.mobile.app/app_flutter
  ...
  static String locale(Function(S s) func,
      {BuildContext? ctx, String defShow = " "}) {
    S? __s = _ss(ctx: ctx);
  ...
  static double screenWidth({BuildContext? context}) =>
      MediaQuery.of(context ?? appContext).size.width;
  static double screenHeight({BuildContext? context}) =>
      MediaQuery.of(context ?? appContext).size.height;
  ...
  static const int gapTopicSubscribeCheckMs =
      10 * 24 * 60 * 60 * 1000; // 10d < 23d
  static const int gapTopicPermissionCheckMs =
      10 * 24 * 60 * 60 * 1000; // 10d < 23d
  ...
  static const int timeoutIpfsThumbnailAutoDownloadMs =
      7 * 24 * 60 * 60 * 1000; // 7d
  // tryTimes
  ...
  static init() async {
    // settings
    var msgDebug = await SettingsStorage.getSettings(
        SettingsStorage.OPEN_DEVELOP_OPTIONS_MESSAGE_DEBUG);
    ...
    // language
    Settings.language =
        (await SettingsStorage.getSettings(SettingsStorage.LOCALE_KEY)) ??
            'auto';
    // public group discovery
    Settings.discoveryBroadcastEnabled = ((await SettingsStorage.getSettings(
                SettingsStorage.DISCOVERY_BROADCAST_ENABLED_KEY)) ??
            'true') ==
        'true';
    Settings.discoveryBroadcastPeriod = int.tryParse(
            (await SettingsStorage.getSettings(
                    SettingsStorage.DISCOVERY_BROADCAST_PERIOD_KEY)) ??
                '1') ??
        1;
    // profile setup
    Settings.profileSetupCompleted = ((await SettingsStorage.getSettings(
                SettingsStorage.PROFILE_SETUP_COMPLETED)) ??
            'false') ==
        'true';
    // authentication
    final isAuth = await SettingsStorage.getSettings(
        SettingsStorage.BIOMETRICS_AUTHENTICATION);
    Settings.biometricsAuthentication =
        (isAuth == null) ? true : ((isAuth is bool) ? isAuth : true);
    logger.i("Settings - init - "
        ...
        "\n language: $language"
        "\n discoveryBroadcastPeriod: $discoveryBroadcastPeriod"
        "\n appName: $appName"
  ...
  // save discovery broadcast enabled
  static Future<void> saveDiscoveryBroadcastEnabled(bool enabled) async {
    Settings.discoveryBroadcastEnabled = enabled;
    await SettingsStorage.setSettings(
        SettingsStorage.DISCOVERY_BROADCAST_ENABLED_KEY, enabled.toString());
  }

  // save discovery broadcast period
  static Future<void> saveDiscoveryBroadcastPeriod(int period) async {
    Settings.discoveryBroadcastPeriod = period;
    await SettingsStorage.setSettings(
        SettingsStorage.DISCOVERY_BROADCAST_PERIOD_KEY, period.toString());
  }

  // save profile setup completed
  static Future<void> saveProfileSetupCompleted(bool completed) async {
    Settings.profileSetupCompleted = completed;
    await SettingsStorage.setSettings(
        SettingsStorage.PROFILE_SETUP_COMPLETED, completed.toString());
  }
}
