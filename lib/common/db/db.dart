...
import 'package:nchat_mobile/common/db/upgrade1to2.dart';
import 'package:nchat_mobile/common/db/upgrade2to3.dart';
import 'package:nchat_mobile/common/db/upgrade3to4.dart';
import 'package:nchat_mobile/common/db/upgrade4to5.dart';
import 'package:nchat_mobile/common/db/upgrade5to6.dart';
import 'package:nchat_mobile/common/db/upgrade6to7.dart';
...
import 'package:nchat_mobile/storages/device_info.dart';
import 'package:nchat_mobile/storages/message.dart';
import 'package:nchat_mobile/storages/message_piece.dart';
import 'package:nchat_mobile/storages/private_group.dart';
import 'package:nchat_mobile/storages/private_group_item.dart';
import 'package:nchat_mobile/storages/session.dart';
import 'package:nchat_mobile/storages/settings.dart';
import 'package:nchat_mobile/storages/subscriber.dart';
...
class DB {
  ...
  // ignore: close_sinks
  StreamController<String?> _upgradeTipController =
  ...
  ParallelQueue contactQueue = ParallelQueue("storage_contact",
      onLog: (log, error) => error ? logger.w(log) : null);
  ParallelQueue deviceInfoQueue = ParallelQueue("storage_deviceInfo",
      onLog: (log, error) => error ? logger.w(log) : null);
  ParallelQueue topicQueue = ParallelQueue("storage_topic",
      onLog: (log, error) => error ? logger.w(log) : null);
  ParallelQueue subscribeQueue = ParallelQueue("storage_subscriber",
      onLog: (log, error) => error ? logger.w(log) : null);
  ParallelQueue privateGroupQueue = ParallelQueue("storage_private_group",
      onLog: (log, error) => error ? logger.w(log) : null);
  ParallelQueue privateGroupItemQueue = ParallelQueue(
      "storage_private_group_item",
      onLog: (log, error) => error ? logger.w(log) : null);
  ParallelQueue messageQueue = ParallelQueue("storage_message",
      onLog: (log, error) => error ? logger.w(log) : null);
  ParallelQueue messagePieceQueue = ParallelQueue("storage_message_piece",
      onLog: (log, error) => error ? logger.w(log) : null);
  ParallelQueue sessionQueue = ParallelQueue("storage_session",
      onLog: (log, error) => error ? logger.w(log) : null);
  ...
  Future _openWithFix(String publicKey, String seed) async {
    ...
    String password =
        seed.isEmpty ? "" : hexEncode(Uint8List.fromList(Hash.sha256(seed)));
    logger.i("DB - open - exists:$exists - publicKey:$publicKey - path:$path");

    if (!Platform.isIOS) {
      database = await _tryOpenDB(path, password,
          publicKey: publicKey, upgradeTip: true);
    } else {
      if (!exists) {
        ...
        if (database != null) {
          SettingsStorage.setSettings(
              "${SettingsStorage.DATABASE_CLEAN_PWD_ON_IOS_14}:$publicKey",
              true); // await
          SettingsStorage.setSettings(
              "${SettingsStorage.DATABASE_RESET_PWD_ON_IOS_16}:$publicKey",
              true); // await
        } else {
        ...
        // FUTURE:GG IOS_152_V2
        bool clean = (await SettingsStorage.getSettings(
                "${SettingsStorage.DATABASE_CLEAN_PWD_ON_IOS_14}:$publicKey")) ??
            false;
        bool reset = (await SettingsStorage.getSettings(
                "${SettingsStorage.DATABASE_RESET_PWD_ON_IOS_16}:$publicKey")) ??
            false;
        if (reset) {
          database = await _tryOpenDB(path, password,
          ...
          try {
            database =
                await _openDB(path, "", publicKey: publicKey, upgradeTip: true);
          } catch (e, st) {
          ...
          if (database != null) {
            ...
            bool copyTemp =
                await _copyDB2Plaintext(path, copyPath, sourcePwd: "");
            if (copyTemp) {
            ...
            Database? _db;
            try {
              _db = await _openDB(path, password,
                  publicKey: publicKey, upgradeTip: true);
            } catch (e) {}
            if (_db != null) {
              await _db.close();
              await SettingsStorage.setSettings(
                  "${SettingsStorage.DATABASE_RESET_PWD_ON_IOS_16}:$publicKey",
                  true);
              await Future.delayed(Duration(milliseconds: 500));
              return await _openWithFix(publicKey, seed);
  ...
  Future<Database?> _tryOpenDB(String path, String password,
      {String publicKey = "", bool upgradeTip = false}) async {
    try {
      return await _openDB(path, password,
          publicKey: publicKey, upgradeTip: upgradeTip);
    } catch (e, st) {
      handleError(e, st);
      _upgradeTipSink.add(null);
    ...
    var db = await openDatabase(
      ...
      onUpgrade: (Database db, int oldVersion, int newVersion) async {
        ...
        if ((v3to4 || (oldVersion == 4)) && (newVersion >= 5)) {
          ...
          await Upgrade4to5.upgradeContact(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          ...
          await Upgrade4to5.createSession(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade4to5.deletesOldTables(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
        }
        ...
        if ((v5to6 || (oldVersion == 6)) && (newVersion >= 7)) {
          await Upgrade6to7.upgradeDeviceInfo(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.upgradeContact(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.upgradeTopic(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.upgradeSubscriber(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.upgradePrivateGroup(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.upgradePrivateGroupItem(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.upgradeMessage(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.upgradeMessagePiece(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.upgradeSession(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade6to7.deletesOldTables(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
        }

        // 7 -> 8
        if (oldVersion <= 7 && newVersion >= 8) {
          await Upgrade7to8.upgradeMessages(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
          await Upgrade7to8.upgradeSession(db,
              upgradeTipSink: upgradeTip ? _upgradeTipSink : null);
        }
      ...
      onOpen: (Database db) async {
        ...
        if (publicKey.isNotEmpty) {
          // version_now
          await SettingsStorage.setSettings(
              "${SettingsStorage.DATABASE_VERSION}:$publicKey",
              version); // await
          // upgrade_at
          for (var i = 1; i <= VERSION_DB_NOW; i++) {
            int? value = await SettingsStorage.getSettings(
                "${SettingsStorage.DATABASE_VERSION_TIME}:$publicKey:$i");
  ...
  Future<bool> needUpgrade(String publicKey) async {
    ...
    int? savedVersion = await SettingsStorage.getSettings(
        "${SettingsStorage.DATABASE_VERSION}:$publicKey");
  ...
  static Future<bool> checkTableExists(Database db, String table) async {
    var count = Sqflite.firstIntValue(await db.query('sqlite_master',
        columns: ['COUNT(*)'],
        where: 'type = ? AND name = ?',
  ...
  Future<bool> _copyDB2Plaintext(String sourcePath, String targetPath,
      {String sourcePwd = ""}) async {
    // source+target
    Database sourceDB;
    Database targetDB;
    try {
      ...
      await sourceDB
          .execute("Attach DATABASE `$targetPath` AS `plaintext` KEY ``");
    } catch (e, st) {
      handleError(e, st);
      return false;
    }
    ...
    try {
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${ContactStorage.tableName}` SELECT * FROM `${ContactStorage.tableName}`");
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${DeviceInfoStorage.tableName}` SELECT * FROM `${DeviceInfoStorage.tableName}`");
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${TopicStorage.tableName}` SELECT * FROM `${TopicStorage.tableName}`");
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${SubscriberStorage.tableName}` SELECT * FROM `${SubscriberStorage.tableName}`");
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${PrivateGroupStorage.tableName}` SELECT * FROM `${PrivateGroupStorage.tableName}`");
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${PrivateGroupItemStorage.tableName}` SELECT * FROM `${PrivateGroupItemStorage.tableName}`");
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${MessageStorage.tableName}` SELECT * FROM `${MessageStorage.tableName}`");
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${MessagePieceStorage.tableName}` SELECT * FROM `${MessagePieceStorage.tableName}`");
      await sourceDB.execute(
          "INSERT INTO `plaintext`.`${SessionStorage.tableName}` SELECT * FROM `${SessionStorage.tableName}`");
    } catch (e) {}
    // close
    try {
  ...
  Future<bool> _copyDB2Encrypted(
      String sourcePath, String targetPath, String targetPwd) async {
    // source+target
    Database sourceDB;
    Database targetDB;
    try {
      ...
      await targetDB
          .execute("Attach DATABASE `$sourcePath` AS `plaintext` KEY ``");
    } catch (e, st) {
      handleError(e, st);
      return false;
    }
    ...
    try {
      await targetDB.execute(
          "INSERT INTO `${ContactStorage.tableName}` SELECT * FROM `plaintext`.`${ContactStorage.tableName}`");
      await targetDB.execute(
          "INSERT INTO `${DeviceInfoStorage.tableName}` SELECT * FROM `plaintext`.`${DeviceInfoStorage.tableName}`");
      await targetDB.execute(
          "INSERT INTO `${TopicStorage.tableName}` SELECT * FROM `plaintext`.`${TopicStorage.tableName}`");
      await targetDB.execute(
          "INSERT INTO `${SubscriberStorage.tableName}` SELECT * FROM `plaintext`.`${SubscriberStorage.tableName}`");
      await targetDB.execute(
          "INSERT INTO `${PrivateGroupStorage.tableName}` SELECT * FROM `plaintext`.`${PrivateGroupStorage.tableName}`");
      await targetDB.execute(
          "INSERT INTO `${PrivateGroupItemStorage.tableName}` SELECT * FROM `plaintext`.`${PrivateGroupItemStorage.tableName}`");
      await targetDB.execute(
          "INSERT INTO `${MessageStorage.tableName}` SELECT * FROM `plaintext`.`${MessageStorage.tableName}`");
      await targetDB.execute(
          "INSERT INTO `${MessagePieceStorage.tableName}` SELECT * FROM `plaintext`.`${MessagePieceStorage.tableName}`");
      await targetDB.execute(
          "INSERT INTO `${SessionStorage.tableName}` SELECT * FROM `plaintext`.`${SessionStorage.tableName}`");
    } catch (e) {}
    // close
    try {
